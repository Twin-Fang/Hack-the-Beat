...
|---|---|---|
| 헤더 | `파티 회고 & 매칭` 안내 | — |
| `다시 만나고 싶은 사람` | 만난 사람 체크박스 목록 · `선택 제출` · 빈 상태 `파티 중에 만난 사람이 없습니다.` | 체크된 사람 아래에 **`다시 만나고 싶은 정도`** 3버튼(`☕ 가볍게` · `🙌 반가웠어요` · `💫 꼭 다시`, 기본 `반가웠어요`). 캐릭터·관심사 표시. 🔴 제출 body는 `picks: [{ targetTagCode, level }]` |
| `서로 선택된 사람` | 💖 카드 · `서로를 다시 만나고 싶어 합니다!` · 빈 상태 문구 | 카드에 **양쪽 정도**(`나 💫 꼭 다시 · 상대 🙌 반가웠어요`), 캐릭터, 공통 관심사. 단독 선택은 어떤 표시도 없음 |
| 하단 | `내 증표함` · `다음 파티 만들기`(→ `/`) | — |
...
🆕 `캐릭터` · `관심사` · `새싹` · `잎` · `꽃` · `공통 관심사` · `다시 만나고 싶은 정도` · `가볍게` · `반가웠어요` · `꼭 다시` · (⬜B) `지금 만났어요`
...
- 🆕 요청: `createParty({ …, hostCharacter, hostInterests })`, `joinParty({ …, character, interests })`, `submitPicks({ participantId, picks: [{ targetTagCode, level }] })`.
...
8. `npm test` · `npm run build` · `npm run lint` 통과.
